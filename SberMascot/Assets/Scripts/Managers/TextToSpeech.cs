...
namespace Managers {
    ...
    public class TextToSpeech : MonoBehaviour {
        ...
        private AudioDataSettings _audioDataSettings;
        ...
        public void Initialize (AudioDataSettings audioSettings) {
            _audioDataSettings = audioSettings;
            _audioDataSettings.onInitialized += ads => audioSource.pitch = ads.SourcePitch;
            _audioDataSettings.Initialize();
        ...
        private IEnumerator AuthenticateSpeechService(Task<string> authenticating) {
            ...
            try {
                _accessToken = authenticating.Result;
                IsReady = true;
                Debug.Log($"Token: {_accessToken}\n");
            } catch (Exception ex) {
        ...
        /// <param name="args">The <see cref="GenericEventArgs{T}"/> instance containing the event data.</param>
        ...
        public void SpeakWithRESTAPI(string message)
        {
            try
            {
                ...
                Task<Stream> Speaking = cortana.Speak(CancellationToken.None, new Synthesize.InputOptions()
                {
                    ...
                    Locale = cortana.GetVoiceLocale(_audioDataSettings.VoiceName),
                    // You can also customize the output voice. Refer to the documentation to view the different
                    // voices that the TTS service can output.
                    VoiceName = _audioDataSettings.VoiceName,

                    // Service can return audio in different output format.
                    OutputFormat = AudioOutputFormat.Riff24Khz16BitMonoPcm,
                    PitchDelta = _audioDataSettings.VoicePitch,
                    AuthorizationToken = "Bearer " + _accessToken,
                });
        ...
        public void SpeakWithSDKPlugin(string message)
        {
            ...
            config.SpeechSynthesisLanguage = cortana.GetVoiceLocale(_audioDataSettings.VoiceName);
            config.SpeechSynthesisVoiceName = cortana.ConvertVoiceNametoString(_audioDataSettings.VoiceName);
        ...
        private IEnumerator WaitAndPlayRoutineSDK(Task<SpeechSynthesisResult> speakTask)
        {
            ...
            if (result.Reason == ResultReason.SynthesizingAudioCompleted)
            {
                ...
                // Convert data to a Unity audio clip
                Debug.Log($"Converting audio data of size {unityData.Length} to Unity audio clip with {sampleCount} samples at frequency {frequency}.");
                var clip = ToClip("Speech", unityData, sampleCount, Mathf.FloorToInt(frequency * _audioDataSettings.Rate));
